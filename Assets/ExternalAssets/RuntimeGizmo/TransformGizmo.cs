...
namespace RuntimeGizmos
{
	...
	{
		public ARInteractionController ARInteractionController;
		#region Fields
		...
		void TransformSelected()
		{
			if(mainTargetRoot != null)
			{
				if(nearAxis != Axis.None && Input.GetMouseButtonDown(0))
				{
					...
					// Leo
					ARInteractionController.FingerPressed();
				}
			}
		}
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					if(transType == TransformType.Move)
					{
						...
						for(int i = 0; i < targetRootsOrdered.Count; i++)
						{
							...
							// TODO Leo: communicate with InteractableObject
							target.GetComponent<InteractableObject>().ClientMoves = true;
